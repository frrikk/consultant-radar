...
export function SettingsMenu({ locale }: SettingsMenuProps) {
  ...
  return (
    <details className="group relative">
      <summary className="flex h-8 w-8 cursor-pointer list-none items-center justify-center rounded-[12px] border border-border bg-background text-foreground transition-colors hover:bg-muted">
        <Settings2Icon className="size-4" />
        <span className="sr-only">{t("radar.page.settings")}</span>
      </summary>

      <div className="absolute right-0 top-[calc(100%+0.5rem)] z-30 w-56 rounded-[16px] border border-border bg-card p-2.5 shadow-lg ring-1 ring-black/5">
